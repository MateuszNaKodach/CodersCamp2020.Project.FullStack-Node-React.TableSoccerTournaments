...
describe('Tournament Details REST API', () => {
  it('POST /rest-api/tournament-details | when command success', async () => {
    ...
    const { body, status } = await agent
      .post('/rest-api/tournament-details')
      .send({ tournamentId: 'SampleTournamentId', tournamentName: 'SampleName' });
  ...
  it('POST /rest-api/tournament-details | when command failure', async () => {
    ...
    const commandPublisher = CommandPublisherMock(
      CommandResult.failureDueTo(new Error('Tournament name must have at least 3 characters.')),
    );
    const { agent } = testModuleRestApi(TournamentDetailsRestApiModule, { commandPublisher });
    ...
    const { body, status } = await agent
      .post('/rest-api/tournament-details')
      .send({ tournamentId: 'SampleTournamentId', tournamentName: 'BB' });

    //Then
    expect(commandPublisher.executeCalls).toBeCalledWith(
      new AddTournamentDetails({ tournamentId: 'SampleTournamentId', tournamentName: 'BB' }),
    );
