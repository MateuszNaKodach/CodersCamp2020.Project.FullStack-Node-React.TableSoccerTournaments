...
export class TournamentTree {
    ...
    public getMatchIdFromMatchNumberInSequence(matchNumberInSequence: number): string {
      return  this.tournamentTreeArray
          .filter(match =>  match.matchNumberInSequence === matchNumberInSequence)[0]?.fightingTeamsGroupId.raw;
    }
}
